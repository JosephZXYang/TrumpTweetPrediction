...
with open('1234.csv', 'r') as train:
    reader = csv.reader(train)
    next(reader)
    for row in reader:
        X.append(row[1]) # Column 1 is the text cell
        yTr.append(int(row[0])) # Column 17 is the label cell
...
clf8 = DecisionTreeClassifier(criterion='gini', max_depth=10)
clf9 = DecisionTreeClassifier(criterion='gini', max_depth=25)
clf10 = DecisionTreeClassifier(criterion='gini', max_depth=5)
...
with open('submission_charword3.csv', mode='w') as submission:
